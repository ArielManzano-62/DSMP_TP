...
class HistorialSeguimientosGrupo extends Component {
    ...
    getHistorialSeguimientos = async () => {
        try {
            const response = await axios.get(`${eventosEndpoint}/api/seguimientos/historial/${this.props.grupoSeleccionado.grupoId}`);
    ...
    renderItem = ({item}) => { 
        ...
        return (
            <ListItem 
                ...
                onPress={() => {
                    this.props.navigation.navigate("DetalleHistorialSeguimientoGrupo", {
                        seguimiento: item
                    })
                }}
        ...
        return (
            <Layout level='2' style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
                ...
                <Text category='s1' appearence='hint' style={{textAlign: 'center'}}>
                    Este grupo no ha tenido seguimientos aún
                </Text>
            </Layout>
        )
...
const Screen = withStyles(HistorialSeguimientosGrupo, (theme) => ({
    headerContainer: {
